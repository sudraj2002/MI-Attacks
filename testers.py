...
def test_vgg(test_loader, model_path, device='cuda', attack=None, targeted=False, targeted_labels=None):
...
def test_vit(model_path, num_classes, test_loader, device='cuda', attack=None, targeted=False, targeted_labels=None):
    model, classification_head = ViT_pretrained(num_classes=num_classes).to(device)
    vit_dict = torch.load(model_path)
